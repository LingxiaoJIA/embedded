...
behavior EdgeDrawThread_PartA(uchar image_buffer[7220], uchar mid[7220], in int thID, OSAPI os) implements Init
{
    ...
    void main(void) {
        ...
        if (drawing_mode==0)
        {
            ...
            {
            //waitfor(12000000);  //insertion for the timing based on the performance profiling
                if (*midp<8) 
                {
...
behavior EdgeDraw_PartA(uchar image_buffer[7220], uchar mid[7220], OSAPI os) implements Init
{
	Task *task;
...
behavior EdgeDraw_PartB(uchar image_buffer[7220], uchar mid[7220], OSAPI os) implements Init
{
	Task *task;
...
behavior Draw(i_uchar7220_receiver in_image, i_uchar7220_receiver in_mid,  i_uchar7220write_sender out_image, OSAPI os)
{
