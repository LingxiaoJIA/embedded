...
behavior SusanThinThread(int r[IMAGE_SIZE], uchar mid[IMAGE_SIZE], in int thID, OSAPI os) implements Init
{
...
//		}
	...
	//
	//
	//waitfor(6400000);  //insertion for the timing based on the performance profiling    

                }

	os.task_terminate();//changed here
    }//main                
};
...
behavior SusanThin(int r[IMAGE_SIZE], uchar mid[IMAGE_SIZE], OSAPI os)  implements Init
{
	Task *me;
//	Task retID;	

    SusanThinThread susan_thin_thread_0(r, mid, 0, os);
    SusanThinThread susan_thin_thread_1(r, mid, 1, os);
    
	void init(void){
		me = os.task_create("st");
	}
...
behavior Thin(i_int7220_receiver in_r, i_uchar7220_receiver in_mid, i_uchar7220_sender out_mid, OSAPI os)
{
	//Task me;
